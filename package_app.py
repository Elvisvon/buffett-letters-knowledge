...
把整个项目封装为自包含的 macOS App + DMG 安装镜像：
  ...
  - 双击 .app → 原生窗口承载应用（Swift + WKWebView），**Dock 图标停留**，
    后台自动拉起本地服务（首选 127.0.0.1:8666，可回退至 8685），关闭窗口即停止
  - 记忆材料（笔记/收藏/已读/AI 对话）默认持久化到
    ~/Library/Application Support/巴菲特投资智慧/state.json（不进 Git）
  - 环境变量密钥只进入服务内存；设置面板手填密钥仅存浏览器 localStorage
...
三、使用
  ...
  （首选 127.0.0.1:8666，端口占用时回退至 8685）。关闭窗口即退出应用并停止服务。
  - 原文链接等外部网页会用系统默认浏览器打开；
  - App 会扫描并复用 8666–8685 上已有的本应用服务。
...
四、LLM 密钥（可选）
  ...
  默认模型 deepseek-v4-flash；环境变量密钥不写入应用状态，手填密钥仅存本机浏览器 localStorage。
